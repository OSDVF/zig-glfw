...
pub fn getProcAddress(proc_name: [*:0]const u8) callconv(.C) ?GLProc {
    internal_debug.assertInitialized();
    if (c.glfwGetProcAddress(proc_name)) |proc_address| return @ptrCast(proc_address); // Must be cast, because Zig could incorrectly import glfwGetProcAddress return type
    return null;
}
